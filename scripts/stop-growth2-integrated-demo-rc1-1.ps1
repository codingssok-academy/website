...
function Stop-VerifiedProcessTree {
    ...
    if ($commandLine -notmatch 'next dev' -or $commandLine -notmatch '3019') {
        throw "기록된 프로세스가 3019 RC1.1 화면 서버인지 확인할 수 없어 종료하지 않았습니다."
    }
    & cmd.exe /d /c "taskkill.exe /PID $ProcessId /T /F 1>nul 2>nul"
    Start-Sleep -Milliseconds 200
    if (Get-Process -Id $ProcessId -ErrorAction SilentlyContinue) {
        throw "3019 RC1.1 화면 서버를 안전하게 종료하지 못했습니다."
    }
}
...
if ($dbRunning) {
    ...
    try {
        Write-Host "Growth 2.0 통합 데모 RC1.1 자료를 초기화합니다."
...
Write-Host "STOPPED growth2_demo_rc1_1=1 port=3019 local_db_reset=1 other_projects_unchanged=1"
