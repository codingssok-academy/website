...
function Stop-VerifiedProcessTree {
    ...
    if ($commandLine -notmatch 'next dev' -or $commandLine -notmatch '3017') {
        throw "기록된 프로세스가 3017 공동 화면 서버인지 확인할 수 없어 종료하지 않았습니다."
    }
    & cmd.exe /d /c "taskkill.exe /PID $ProcessId /T /F 1>nul 2>nul"
    Start-Sleep -Milliseconds 200
    if (Get-Process -Id $ProcessId -ErrorAction SilentlyContinue) {
        throw "3017 공동 화면 서버를 안전하게 종료하지 못했습니다."
    }
}
...
if (Test-Path $pidPath -PathType Leaf) {
    ...
    if (-not [int]::TryParse((Get-Content -Raw $pidPath).Trim(), [ref]$savedPid)) {
        throw "공동 화면 서버 번호 파일을 읽지 못했습니다."
    }
...
if (Test-Path $backendRoot -PathType Container) {
    ...
    try {
        Write-Host "학생·학부모·선생님 공동 연습 DB 자료를 초기화합니다."
        ...
        Write-Host "이 공동 미리보기용 Supabase만 종료합니다."
...
foreach ($environmentFile in @(".env", ".env.local")) {
    if (Test-Path (Join-Path $frontendRoot $environmentFile) -PathType Leaf) {
        throw "공동 화면 작업 폴더에 금지된 환경 파일이 있습니다: $environmentFile"
    }
}
...
Write-Host "STOPPED local_student_shared_preview=1 local_db_reset=1 auth_users=0 students=0 evaluations=0 xp=0 missions=0 badges=0 mission_catalog=3 badge_catalog=3"
