...
function Stop-VerifiedProcessTree {
    ...
    if ($commandLine -notmatch 'next dev' -or $commandLine -notmatch '3013') {
        throw "기록된 프로세스가 3013 선생님 화면 서버인지 확인할 수 없어 종료하지 않았습니다."
    }
...
if (Test-Path $backendRoot -PathType Container) {
    ...
    try {
        ...
        $sql = "select (select count(*) from auth.users),(select count(*) from public.growth_students),(select count(*) from public.growth_weekly_evaluations),(select count(*) from public.growth_weekly_evaluation_concepts),(select count(*) from public.growth_projects),(select count(*) from public.growth_xp_transactions),(select count(*) from public.growth_activity_events),(select count(*) from public.growth_student_missions),(select count(*) from public.growth_missions),(select count(*) from public.growth_badges);"
        $counts = (& docker exec supabase_db_codingssok-growth-v2-local psql -U postgres -d postgres -At -F ',' -c $sql | Out-String).Trim()
        if ($LASTEXITCODE -ne 0 -or $counts -ne "0,0,0,0,0,0,0,0,3,3") {
...
Write-Host "STOPPED local_teacher_preview=1 local_db_reset=1 auth_users=0 students=0 evaluations=0 mission_catalog=3 badge_catalog=3"
