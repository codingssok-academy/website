...
$previewUrl = "http://127.0.0.1:3014/growth-preview/teacher-local"
...
try {
    ...
    $portOwner = Get-NetTCPConnection -LocalPort 3014 -State Listen -ErrorAction SilentlyContinue
    if ($null -ne $portOwner) {
        throw "3014번 포트를 다른 프로그램이 사용 중입니다. 해당 프로그램을 먼저 종료해 주세요."
    }
    ...
    Write-Host "[3/4] 3014번 직접 입력 개념 화면을 시작합니다."
    ...
    $process = Start-Process -FilePath "cmd.exe" `
        -ArgumentList "/d", "/c", "npx.cmd next dev --webpack -H 127.0.0.1 -p 3014" `
    ...
    Write-Host "READY url=$previewUrl local_db=1 teacher=1 students=2 draft_save=1 custom_concepts=1 published_write=0"
}
finally {
