...
$backendRoot = "C:\dev\codingssok-growth-v2-backend-teacher-preview"
$dependencyRoot = "C:\dev\codingssok-growth-v2\node_modules"
$stateRoot = Join-Path $env:TEMP "codingssok-growth-v2-teacher-draft-local"
...
$previewUrl = "http://127.0.0.1:3013/growth-preview/teacher-local"
...
try {
    if (-not (Test-Path $backendRoot -PathType Container)) {
        throw "백엔드 미리보기 폴더를 찾지 못했습니다: $backendRoot"
    }
    if (-not (Test-Path (Join-Path $backendRoot "scripts\prepare-local-teacher-draft-preview.ps1") -PathType Leaf)) {
        throw "가상 선생님 자료 준비 스크립트를 찾지 못했습니다."
    }
    if (-not (Test-Path (Join-Path $frontendRoot "src\app\growth-preview\teacher-local\page.tsx") -PathType Leaf)) {
        throw "로컬 선생님 화면 파일을 찾지 못했습니다."
    }
    ...
    $portOwner = Get-NetTCPConnection -LocalPort 3013 -State Listen -ErrorAction SilentlyContinue
    if ($null -ne $portOwner) {
        throw "3013번 포트를 다른 프로그램이 사용 중입니다. 해당 프로그램을 먼저 종료해 주세요."
    }

    Write-Host "[1/4] 로컬 선생님 연습 DB를 시작합니다."
    [void](Invoke-CapturedCommand $backendRoot "npx.cmd supabase start" "로컬 연습 DB를 시작하지 못했습니다.")

    Write-Host "[2/4] DB를 초기화하고 가상 선생님과 학생 A·B를 준비합니다."
    ...
    try {
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".\scripts\prepare-local-teacher-draft-preview.ps1"
        if ($LASTEXITCODE -ne 0) { throw "가상 선생님 자료 준비에 실패했습니다." }
    }
    finally { Pop-Location }

    Write-Host "[3/4] 3013번 로컬 선생님 화면을 시작합니다."
    ...
    $process = Start-Process -FilePath "cmd.exe" `
        -ArgumentList "/d", "/c", "npx.cmd next dev --webpack -H 127.0.0.1 -p 3013" `
    ...
    for ($attempt = 0; $attempt -lt 90; $attempt++) {
        ...
        if ($process.HasExited) { throw "선생님 화면 서버가 시작 중 종료되었습니다. 임시 로그를 확인해 주세요." }
        try {
    ...
    if (-not $ready) { throw "선생님 화면이 제한 시간 안에 준비되지 않았습니다." }

    Write-Host "[4/4] 준비가 끝났습니다. 브라우저를 엽니다."
    Start-Process $previewUrl
    Write-Host "READY url=$previewUrl local_db=1 teacher=1 students=2 draft_save=1 published_write=0"
}
finally {
