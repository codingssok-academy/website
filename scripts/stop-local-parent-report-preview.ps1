...
function Stop-VerifiedProcessTree {
    ...
    if ($commandLine -notmatch 'next dev' -or $commandLine -notmatch '3016') {
        throw "기록된 프로세스가 3016 학부모 화면 서버인지 확인할 수 없어 종료하지 않았습니다."
    }
    & cmd.exe /d /c "taskkill.exe /PID $ProcessId /T /F 1>nul 2>nul"
    Start-Sleep -Milliseconds 200
    if (Get-Process -Id $ProcessId -ErrorAction SilentlyContinue) {
        throw "3016 화면 서버를 안전하게 종료하지 못했습니다."
    }
}
...
if (Test-Path $pidPath -PathType Leaf) {
    ...
    if (-not [int]::TryParse((Get-Content -Raw $pidPath).Trim(), [ref]$savedPid)) {
        throw "학부모 화면 서버 번호 파일을 읽지 못했습니다."
    }
...
Write-Host "STOPPED local_parent_report_preview=1 local_db_reset=1 auth_users=0 students=0 evaluations=0 custom_concepts=0 mission_catalog=3 badge_catalog=3"
