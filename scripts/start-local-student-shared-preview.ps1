...
$backendRoot = "C:\dev\codingssok-growth-v2-backend-student-shared-preview"
$dependencyRoot = "C:\dev\codingssok-growth-v2\node_modules"
$stateRoot = Join-Path $env:TEMP "codingssok-growth-v2-student-shared-local"
...
$studentUrl = "http://127.0.0.1:3017/growth-preview/student-local"
$parentUrl = "http://127.0.0.1:3017/growth-preview/parent-local"
$teacherUrl = "http://127.0.0.1:3017/growth-preview/teacher-local"
...
try {
    if (-not (Test-Path $backendRoot -PathType Container)) {
        throw "공동 학생 미리보기 백엔드 폴더를 찾지 못했습니다: $backendRoot"
    }
    if (-not (Test-Path (Join-Path $backendRoot "scripts\prepare-local-student-shared-preview.ps1") -PathType Leaf)) {
        throw "공동 가상 학생 자료 준비 스크립트를 찾지 못했습니다."
    }
    foreach ($page in @(
        "src\app\growth-preview\student-local\page.tsx",
        "src\app\growth-preview\parent-local\page.tsx",
        "src\app\growth-preview\teacher-local\page.tsx"
    )) {
        if (-not (Test-Path (Join-Path $frontendRoot $page) -PathType Leaf)) {
            throw "3017 공동 화면 파일을 찾지 못했습니다: $page"
        }
    }
    ...
    $portOwner = Get-NetTCPConnection -LocalPort 3017 -State Listen -ErrorAction SilentlyContinue
    if ($null -ne $portOwner) {
        throw "3017번 포트를 다른 프로그램이 사용 중입니다. 해당 프로그램을 먼저 종료해 주세요."
    }

    Write-Host "[1/4] 학생·학부모·선생님 공동 로컬 연습 DB를 시작합니다."
    [void](Invoke-CapturedCommand $backendRoot "npx.cmd supabase start" "로컬 연습 DB를 시작하지 못했습니다.")

    Write-Host "[2/4] DB를 초기화하고 네 가상 계정과 학생 학습 자료를 준비합니다."
    ...
    try {
        & powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".\scripts\prepare-local-student-shared-preview.ps1"
        if ($LASTEXITCODE -ne 0) { throw "공동 가상 학생 자료 준비에 실패했습니다." }
    }
    finally { Pop-Location }

    Write-Host "[3/4] 3017번 학생·학부모·선생님 화면을 시작합니다."
    ...
    $process = Start-Process -FilePath "cmd.exe" `
        -ArgumentList "/d", "/c", "npx.cmd next dev --webpack -H 127.0.0.1 -p 3017" `
    ...
    for ($attempt = 0; $attempt -lt 100; $attempt++) {
        ...
        if ($process.HasExited) { throw "공동 화면 서버가 시작 중 종료되었습니다. 임시 로그를 확인해 주세요." }
        try {
            $studentResponse = Invoke-WebRequest -Uri $studentUrl -UseBasicParsing -TimeoutSec 2
            ...
            if ($studentResponse.StatusCode -eq 200 -and $parentResponse.StatusCode -eq 200 -and $teacherResponse.StatusCode -eq 200) {
                $ready = $true; break
            }
        }
        catch { }
    }
    if (-not $ready) { throw "공동 화면이 제한 시간 안에 준비되지 않았습니다." }

    Write-Host "[4/4] 준비가 끝났습니다. 학생 화면을 엽니다."
    Start-Process $studentUrl
    Write-Host "READY student_url=$studentUrl parent_url=$parentUrl teacher_url=$teacherUrl local_db=1 students=2 student_a_published=1 student_a_draft_hidden=1 student_b_empty=1 student_b_draft_hidden=1 read_only=1"
}
finally {
