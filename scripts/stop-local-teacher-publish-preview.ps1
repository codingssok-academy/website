...
function Stop-VerifiedProcessTree {
    ...
    if ($commandLine -notmatch 'next dev' -or $commandLine -notmatch '3015') {
        throw "기록된 프로세스가 3015 평가 공개 화면 서버인지 확인할 수 없어 종료하지 않았습니다."
    }
    & cmd.exe /d /c "taskkill.exe /PID $ProcessId /T /F 1>nul 2>nul"
    Start-Sleep -Milliseconds 200
    if (Get-Process -Id $ProcessId -ErrorAction SilentlyContinue) {
        throw "3015 화면 서버를 안전하게 종료하지 못했습니다."
    }
}
...
Write-Host "STOPPED local_teacher_publish_preview=1 local_db_reset=1 auth_users=0 students=0 evaluations=0 custom_concepts=0 mission_catalog=3 badge_catalog=3"
