...
describe("local student report", () => {
  ...
  it("blocks duplicate login and does not use browser storage", async () => {
    ...
    release?.(SESSION);
    expect(await screen.findByRole("heading", { name: "테스트 학생 A의 성장 리포트" })).toBeInTheDocument();
  ...
  it("reloads only through the student home reader and shows A version 2 custom concepts", async () => {
    ...
    render(<LocalStudentReport />);
    fireEvent.click(screen.getByRole("button", { name: "테스트 학생 A로 들어가기" }));
    await screen.findByText(/질문을 정리하며 문제를 해결/);
    ...
    release?.(A_V2);
    expect(await screen.findByText("오류 찾기")).toBeInTheDocument();
  ...
  it("clears all in-memory student data on exit and reads again after login", async () => {
    render(<LocalStudentReport />);
    fireEvent.click(screen.getByRole("button", { name: "테스트 학생 A로 들어가기" }));
    await screen.findByRole("heading", { name: "테스트 학생 A의 성장 리포트" });
  ...
  it("shows local guidance and hides demo navigation in staging mode", async () => {
    const view = render(<LocalStudentReport />);
    expect(screen.getByText("Growth 2.0 로컬 연습 환경")).toBeInTheDocument();
    expect(screen.getByText(/실제 학생 정보가 아닌 로컬 테스트 자료/)).toBeInTheDocument();
    expect(screen.getByRole("link", { name: "기존 데모 학생 화면 보기" })).toBeInTheDocument();
    view.unmount();
    ...
    expect(screen.queryByRole("link", { name: /기존 데모 학생 화면/ })).not.toBeInTheDocument();
    fireEvent.click(screen.getByRole("button", { name: "테스트 학생 A로 들어가기" }));
    expect(await screen.findByRole("heading", { name: "테스트 학생 A의 성장 리포트" })).toBeInTheDocument();
    expect(screen.queryByRole("navigation", { name: "데모 화면 이동" })).not.toBeInTheDocument();
  });
});
